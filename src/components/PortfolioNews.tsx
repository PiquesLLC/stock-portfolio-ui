...
const SENTIMENT_STYLES: Record<string, { color: string; label: string }> = {
  ...
  neutral: { color: 'text-rh-light-text dark:text-white', label: 'Neutral' },
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-rh-light-text dark:text-white">Powered by NALA AI</span>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={onRefresh}
            disabled={refreshing}
            className="text-rh-light-text dark:text-white hover:text-rh-light-text dark:hover:text-rh-text transition-colors disabled:opacity-30"
      ...
      {/* Portfolio Impact */}
      <p className="text-xs text-rh-light-text dark:text-white leading-relaxed mb-3">
        <RichText text={summary.portfolioImpact} tickers={tickers} onTickerClick={onTickerClick} />
      </p>

      {/* Outlook */}
      <p className="text-xs text-rh-light-text dark:text-white leading-relaxed mb-4 italic">
  ...
  if (error && !data) {
    return (
      <div className="py-12 text-center">
        ...
        <p className="text-xs text-rh-light-text dark:text-white mb-3">{error}</p>
  ...
  if (!data || data.items.length === 0) {
    return (
      <div className="py-12 text-center">
        ...
        <p className="text-sm text-rh-light-text dark:text-white">No market data available</p>
        <p className="text-xs text-rh-light-text dark:text-white mt-1">Add holdings to see your personalized analysis</p>
      </div>
    );
  }
    ...
    <div>
      {data.summary ? (
        ...
        <div className="py-12 text-center">
          ...
          <p className="text-sm text-rh-light-text dark:text-white">Generating your market analysis...</p>
        </div>
      )}
            ...
            <div className="space-y-2">
              {mentionCounts.map(({ ticker, count }) => (
                ...
                >
                  ...
                  <span className="text-[10px] font-medium tabular-nums text-rh-light-text dark:text-white w-6 text-right">{count}</span>
                </button>
              ))}
            </div>
          </div>
