...
interface Props {
  ...
  /** Optional controlled state for the Compare:SPY toggle. When provided,
   * overrides the internal state — used by App.tsx so the toggle button
   * can be relocated to the HOLDINGS toolbar on desktop. */
...
export function PortfolioValueChart({
  ...
  showBenchmark: showBenchmarkProp,
  ...
  const [showBenchmarkInternal, setShowBenchmarkInternal] = useState(false);
  // Controlled-or-uncontrolled hybrid: when App.tsx provides external state,
  // the relocated Compare:SPY button (rendered in the HOLDINGS toolbar) drives
  // the toggle. Otherwise fall back to the legacy in-strip button's state.
  const showBenchmark = showBenchmarkProp ?? showBenchmarkInternal;
  const setShowBenchmark = (next: boolean | ((prev: boolean) => boolean)) => {
    const value = typeof next === 'function' ? next(showBenchmark) : next;
    if (onShowBenchmarkChange) onShowBenchmarkChange(value);
    else setShowBenchmarkInternal(value);
  };
