...
function friendlyError(msg: string): string {
  if (msg === 'WAITLIST_NOT_APPROVED') return 'This email is not approved for account creation yet. Use the approved email address or contact support@nalaai.com.';
  return msg;
}
...
export function LandingPage() {
  ...
  // Detect /invite path (waitlist approval emails), ?approved=1, or legacy #signup hash to bypass waitlist gate
  useEffect(() => {
    ...
    <div className="h-screen h-dvh bg-black text-white overflow-hidden flex flex-col" style={{ paddingTop: 'env(safe-area-inset-top, 0px)' }}>
      ...
      <nav className="relative z-40 bg-black border-b border-white/[0.04] shrink-0">
        <div className="max-w-6xl mx-auto px-5 sm:px-8 h-14 flex items-center justify-between">
          ...
          <div className="hidden sm:flex items-center gap-8 absolute left-1/2 -translate-x-1/2">
            <button onClick={() => scrollToRef(featuresRef)} className="text-[13px] text-white/80 hover:text-white transition-colors">Features</button>
            <button onClick={() => scrollToRef(pricingRef)} className="text-[13px] text-white/80 hover:text-white transition-colors">Pricing</button>
            <button onClick={() => scrollToRef(faqRef)} className="text-[13px] text-white/80 hover:text-white transition-colors">FAQ</button>
          </div>
          <div className="hidden sm:flex items-center gap-4">
            <button onClick={() => openAuth('login')} className="text-[13px] text-white hover:text-white transition-colors">Log in</button>
          ...
          <div className="flex sm:hidden items-center gap-3">
            <button onClick={() => openAuth('login')} className="text-[13px] text-white">Log in</button>
      ...
      <section className="pt-6 sm:pt-12 pb-16 sm:pb-24 px-5 sm:px-8">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="text-center">
            ...
            <p className="text-sm sm:text-base text-white max-w-lg mx-auto mb-8 leading-relaxed">
              Track your portfolio, follow top investors, and get AI-powered insights — all in one place.
            </p>
            <div className="flex items-center justify-center gap-3">
              <button onClick={() => scrollToRef(featuresRef)} className="px-6 py-2.5 text-[13px] text-white font-medium border border-white/[0.12] rounded-full hover:border-white/25 hover:text-white transition-all">Learn More</button>
      ...
      <section ref={featuresRef} className="py-20 sm:py-28 bg-[#080808]">
        <div className="max-w-6xl mx-auto px-5 sm:px-8">
          ...
          <div className="text-center mb-16 sm:mb-20">
            ...
            <p className="text-sm sm:text-base text-white max-w-xl mx-auto">Follow top-performing investors, run AI-powered research, and see what Wall Street won't show you.</p>
          </div>
                  ...
                  <div className="relative rounded-2xl border border-white/[0.08] bg-[#0a0a0a] overflow-hidden transition-colors duration-500 group-hover:border-white/[0.12]">
                    ...
                    <div className="grid lg:grid-cols-2">
                      ...
                      <div className={`flex flex-col justify-center p-8 sm:p-10 lg:p-12 order-2 ${!flipped ? 'lg:order-1' : ''}`}>
                        ...
                        <p className="text-sm sm:text-[15px] text-white leading-relaxed">{f.desc}</p>
                      </div>
                      {/* Screenshot */}
          ...
          <div className="mt-16 text-center">
            ...
            <p className="text-[11px] text-white/80 mt-3">Free forever. Upgrade anytime.</p>
          </div>
        </div>
      </section>
      ...
      {lightbox !== null && (
        ...
        >
          ...
          >
            ...
            <button onClick={() => setLightbox((lightbox - 1 + ALL_FEATURES.length) % ALL_FEATURES.length)} className="absolute left-[-3rem] top-1/2 -translate-y-1/2 text-white hover:text-white transition-colors hidden sm:block">
              <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
            </button>
            <button onClick={() => setLightbox((lightbox + 1) % ALL_FEATURES.length)} className="absolute right-[-3rem] top-1/2 -translate-y-1/2 text-white hover:text-white transition-colors hidden sm:block">
            ...
            <div className="mt-3 sm:mt-4 text-center px-4 shrink-0">
              ...
              <p className="text-xs sm:text-sm text-white mt-1">{ALL_FEATURES[lightbox].desc}</p>
            </div>
          </div>
        </div>
      )}
      ...
      <section className="py-20 sm:py-28 px-5 sm:px-8">
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-center text-sm text-white mb-14 max-w-lg mx-auto">Bank-level security to protect your data and privacy.</p>
          <div className="grid md:grid-cols-2 gap-8 items-start">
            <div className="rounded-2xl border border-white/[0.06] bg-[#0b0b0b] p-8 flex flex-col items-center justify-center">
              ...
              <div className="text-sm text-white text-center font-medium">AES-256 Encryption</div><div className="text-[11px] text-white/80 text-center mt-1">Data encrypted at rest and in transit</div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
              {[{t:'Multi-Factor Auth',d:'TOTP, email OTP, and backup codes'},{t:'Encrypted Storage',d:'AES-256-GCM for all sensitive data'},{t:'Secure Cookies',d:'httpOnly, SameSite, Secure flags'},{t:'Rate Limiting',d:'Per-user API rate limits'},{t:'CORS Protection',d:'Strict origin whitelisting'},{t:'Plaid Security',d:'Tokens encrypted, never exposed'}].map(f=>(
                <div key={f.t} className="flex items-start gap-2.5">
                  ...
                  <div><div className="text-[13px] font-medium text-white/80">{f.t}</div><div className="text-[11px] text-white/80">{f.d}</div></div>
                </div>
              ))}
            </div>
          </div>
      ...
      <section ref={pricingRef} className="py-20 sm:py-28 px-5 sm:px-8 bg-[#080808]">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-14">
            ...
            <p className="text-sm text-white/80 max-w-sm mx-auto mb-8">Start free. Upgrade when you need more power.</p>
            <div className="inline-flex items-center gap-1 p-1 rounded-full bg-white/[0.04] border border-white/[0.06]">
              <button onClick={() => setBilling('monthly')} className={`px-4 py-1.5 rounded-full text-[12px] font-medium transition-all ${billing === 'monthly' ? 'bg-white text-black' : 'text-white hover:text-white'}`}>Monthly</button>
              <button onClick={() => setBilling('yearly')} className={`px-4 py-1.5 rounded-full text-[12px] font-medium transition-all ${billing === 'yearly' ? 'bg-white text-black' : 'text-white hover:text-white'}`}>Yearly <span className="text-rh-green ml-1">-35%</span></button>
            </div>
          </div>
          <div className="hidden sm:grid grid-cols-4 gap-4 mb-8">
            {PLANS.map(plan => { const price = billing === 'yearly' ? plan.yearlyPrice : plan.monthlyPrice; const isElite = plan.id === 'elite'; return (
              <div key={plan.id} className={`relative rounded-2xl p-6 flex flex-col border transition-all ${plan.highlight ? 'border-rh-green/20 bg-white/[0.03]' : isElite ? 'border-purple-500/20 bg-white/[0.03]' : 'border-white/[0.06] bg-white/[0.015]'}`}>
                ...
                <div className="mb-5"><div className="flex items-center gap-2 mb-3"><h3 className={`text-sm font-semibold ${isElite ? 'text-purple-400/80' : 'text-white/80'}`}>{plan.name}</h3>{plan.highlight && <span className="px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider bg-rh-green/15 text-rh-green">Popular</span>}{isElite && <span className="px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider bg-purple-500/15 text-purple-400">Research</span>}</div><div className="flex items-baseline gap-1"><span className="text-3xl font-bold text-white">${price}</span>{price > 0 && <span className="text-sm text-white/80">/{billing === 'yearly' ? 'yr' : 'mo'}</span>}</div><p className="text-xs text-white/80 mt-2">{plan.description}</p></div>
                <ul className="flex-1 space-y-2.5 mb-6">{plan.features.map(f => <li key={f} className="flex items-start gap-2 text-[13px]"><CheckIcon className="mt-0.5" /><span className="text-white/80">{f}</span></li>)}</ul>
                <button onClick={() => openAuth(ctaMode)} className={`w-full py-2.5 rounded-full text-[13px] font-medium transition-all min-h-[44px] ${plan.highlight ? 'bg-white text-black hover:bg-white/90' : isElite ? 'bg-gradient-to-r from-purple-500/80 to-purple-600/80 text-white hover:from-purple-500 hover:to-purple-600' : 'border border-white/[0.1] text-white/80 hover:border-white/20'}`}>Get Started</button>
              </div>
            ); })}
          </div>
              ...
              {PLANS.map((plan, idx) => { const price = billing === 'yearly' ? plan.yearlyPrice : plan.monthlyPrice; const isElite = plan.id === 'elite'; return (
                <div key={plan.id} className="snap-center shrink-0 w-[calc(100vw-56px)]">
                  <div className={`rounded-2xl p-6 flex flex-col min-h-[380px] border transition-all duration-300 ${plan.highlight ? 'border-rh-green/20 bg-white/[0.03]' : isElite ? 'border-purple-500/20 bg-white/[0.03]' : 'border-white/[0.06] bg-white/[0.015]'} ${activeSlide === idx ? 'opacity-100' : 'opacity-50'}`}>
                    ...
                    <div className="flex items-baseline gap-1 mb-1"><span className="text-3xl font-bold text-white">${price}</span>{price > 0 && <span className="text-sm text-white/80">/{billing === 'yearly' ? 'yr' : 'mo'}</span>}</div>
                    <p className="text-xs text-white/80 mb-5">{plan.description}</p>
                    <ul className="flex-1 space-y-2 mb-5">{plan.features.map(f => <li key={f} className="flex items-start gap-2 text-[13px]"><CheckIcon className="mt-0.5" /><span className="text-white/80">{f}</span></li>)}</ul>
                    <button onClick={() => openAuth(ctaMode)} className={`w-full py-2.5 rounded-full text-[13px] font-medium min-h-[44px] ${plan.highlight ? 'bg-white text-black' : isElite ? 'bg-gradient-to-r from-purple-500/80 to-purple-600/80 text-white' : 'border border-white/[0.1] text-white/80'}`}>Get Started</button>
                  </div>
                </div>
              ); })}
            </div>
      ...
      <section className="py-20 sm:py-28 px-5 sm:px-8 text-center">
        ...
        <p className="text-sm text-white/80 mb-10 max-w-sm mx-auto">{DIRECT_SIGNUP_ENABLED ? 'Create your free account, add your holdings, and follow top investors.' : 'Join the waitlist, get approved, and start tracking your portfolio.'}</p>
        <div className="flex items-center justify-center gap-3 mb-10">
          {(DIRECT_SIGNUP_ENABLED ? ['Sign up','Add holdings','Start earning'] : ['Join waitlist','Get approved','Start earning']).map((step,i)=>(
            <div key={step} className="flex items-center gap-3">
              <div className="flex items-center gap-2"><div className={`w-6 h-6 rounded-full flex items-center justify-center text-[9px] font-bold ${i===2?'bg-rh-green/20 text-rh-green':'bg-white/[0.05] text-white'}`}>{i+1}</div><span className="text-[11px] text-white/80">{step}</span></div>
        ...
        <div className="max-w-5xl mx-auto"><div className="grid md:grid-cols-5 gap-10 md:gap-16">
          ...
          <div className="md:col-span-3"><div className="border border-white/[0.06] rounded-2xl overflow-hidden">{FAQ_ITEMS.map((item, i) => (
            <div key={i} className={i > 0 ? 'border-t border-white/[0.04]' : ''}>
              <button onClick={() => setOpenFaq(openFaq === i ? null : i)} className="w-full flex items-center justify-between px-5 py-4 text-left group"><span className="text-[13px] font-medium text-white/80 group-hover:text-white transition-colors pr-4">{item.q}</span><svg className={`w-4 h-4 text-white/80 shrink-0 transition-transform duration-200 ${openFaq === i ? 'rotate-45' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg></button>
              <div className={`overflow-hidden transition-all duration-200 ${openFaq === i ? 'max-h-40 pb-4' : 'max-h-0'}`}><p className="px-5 text-[12px] text-white/80 leading-relaxed">{item.a}</p></div>
            </div>
          ))}</div></div>
        </div></div>
      </section>
      ...
      <footer className="border-t border-white/[0.04] py-12 px-5 sm:px-8"><div className="max-w-5xl mx-auto">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-10">
          <div className="col-span-2 md:col-span-1"><div className="flex items-center gap-2 mb-3"><img src="/north-signal-logo.png" alt="" className="h-6 w-6" /><span className="text-sm font-bold text-white/80">Nala</span></div><p className="text-[11px] text-white/80 leading-relaxed">AI-powered social investing platform for the next generation.</p></div>
          <div><h4 className="text-[11px] font-semibold text-white uppercase tracking-wider mb-3">Product</h4><div className="space-y-2">{['Portfolio','Heatmap','AI Insights','Leaderboard','Dividends'].map(l=><button key={l} onClick={()=>scrollToRef(featuresRef)} className="block text-[12px] text-white/80 hover:text-white transition-colors">{l}</button>)}</div></div>
          <div><h4 className="text-[11px] font-semibold text-white uppercase tracking-wider mb-3">Company</h4><div className="space-y-2"><button onClick={()=>scrollToRef(pricingRef)} className="block text-[12px] text-white/80 hover:text-white transition-colors">Pricing</button><a href="/support" className="block text-[12px] text-white/80 hover:text-white transition-colors">Support</a><button onClick={()=>{setPrivacyTab('privacy');setShowPrivacyPolicy(true);}} className="block text-[12px] text-white/80 hover:text-white transition-colors">Privacy Policy</button><button onClick={()=>{setPrivacyTab('terms');setShowPrivacyPolicy(true);}} className="block text-[12px] text-white/80 hover:text-white transition-colors">Terms of Service</button></div></div>
          <div><h4 className="text-[11px] font-semibold text-white uppercase tracking-wider mb-3">Resources</h4><div className="space-y-2"><button onClick={()=>scrollToRef(faqRef)} className="block text-[12px] text-white/80 hover:text-white transition-colors">FAQ</button><button onClick={()=>openAuth(ctaMode)} className="block text-[12px] text-white/80 hover:text-white transition-colors">Get Started</button></div></div>
        </div>
        <div className="flex flex-col sm:flex-row items-center justify-between pt-6 border-t border-white/[0.04] gap-3"><div className="text-[10px] text-white/80">&copy; 2026 Nala AI. All rights reserved.</div><p className="text-[10px] text-white/80 text-center">Past performance does not guarantee future results. Not financial advice.</p></div>
      </div></footer>
      ...
      {authOpen && createPortal(
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4" role="dialog" aria-modal="true">
          ...
          <div className="relative w-full max-w-sm bg-[#0e0e0e] rounded-2xl p-6 shadow-2xl border border-white/[0.06]">
            <button onClick={() => closeAuth()} className="absolute top-3 right-3 p-1.5 text-white/80 hover:text-white/80 transition-colors" aria-label="Close"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button>
              ...
              <>
                ...
                <form onSubmit={handleSubmit} noValidate><div className="space-y-4">
                  {authMode === 'waitlist' ? (waitlistSuccess ? (
                    <div className="text-center py-4">
                      ...
                      <p className="text-white text-[12px] leading-relaxed">We'll email you when access opens.</p>
                      <div className="mt-5"><button type="button" onClick={() => closeAuth()} className="text-[12px] text-white hover:text-white transition-colors">Close</button></div>
                    </div>
                  ) : (<>
                    <p className="text-[12px] text-white leading-relaxed">Enter your email to join the waitlist. We'll notify you when your spot opens.</p>
                    <div><label htmlFor="auth-waitlist-email" className="block text-[12px] font-medium text-white mb-1.5">Email</label><input id="auth-waitlist-email" type={EMAIL_INPUT_TYPE} inputMode="email" value={waitlistEmail} onChange={e=>setWaitlistEmail(e.target.value)} className={ic} placeholder="you@example.com" autoComplete="email" autoFocus required /></div>
                    ...
                    <div className="text-center"><button type="button" onClick={()=>setAuthMode('login')} className="text-[12px] text-white/80">Have an account? <span className="text-white hover:text-white">Sign in</span></button></div>
                  </>)) : authMode === 'forgot-username' ? (<>
                    <p className="text-[12px] text-white leading-relaxed">Enter the email on your account and we'll send your username.</p>
                    <div><label htmlFor="auth-username-email" className="block text-[12px] font-medium text-white mb-1.5">Email</label><input id="auth-username-email" type={EMAIL_INPUT_TYPE} inputMode="email" value={resetEmail} onChange={e=>setResetEmail(e.target.value)} className={ic} placeholder="you@example.com" autoComplete="email" autoFocus required /></div>
                    ...
                    <div className="text-center"><button type="button" onClick={()=>{setAuthMode('login');setError('');}} className="text-[12px] text-white hover:text-white transition-colors">Back to sign in</button></div>
                  </>) : authMode === 'forgot-password' ? (<>
                    <p className="text-[12px] text-white leading-relaxed">Enter the email on your account and we'll send a reset code.</p>
                    <div><label htmlFor="auth-reset-email" className="block text-[12px] font-medium text-white mb-1.5">Email</label><input id="auth-reset-email" type="text" inputMode="email" value={resetEmail} onChange={e=>setResetEmail(e.target.value)} className={ic} placeholder="you@example.com" autoComplete="email" autoCapitalize="none" autoCorrect="off" autoFocus required /></div>
                    ...
                    <p className="text-[12px] text-white leading-relaxed">Enter the 6-digit code sent to <span className="text-white font-medium">{resetEmail}</span>.</p>
                    <div><label htmlFor="auth-reset-code" className="block text-[12px] font-medium text-white mb-1.5">Reset Code</label><input id="auth-reset-code" type="text" inputMode="numeric" maxLength={6} value={resetCode} onChange={e=>setResetCode(e.target.value.replace(/\D/g,'').slice(0,6))} className={`${ic} text-center text-xl tracking-[0.3em] font-mono`} placeholder="000000" autoComplete="one-time-code" autoFocus required /></div>
                    <div><label htmlFor="auth-new-pw" className="block text-[12px] font-medium text-white mb-1.5">New Password</label><input id="auth-new-pw" type="password" value={newPassword} onChange={e=>setNewPassword(e.target.value)} className={ic} placeholder="Min. 8 chars, upper/lower/number" autoComplete="new-password" required /></div>
                    <div><label htmlFor="auth-new-pw-confirm" className="block text-[12px] font-medium text-white mb-1.5">Confirm Password</label><input id="auth-new-pw-confirm" type="password" value={newPasswordConfirm} onChange={e=>setNewPasswordConfirm(e.target.value)} className={ic} placeholder="Re-enter new password" autoComplete="new-password" required /></div>
                    ...
                    <div className="flex items-center justify-between"><button type="button" onClick={async()=>{if(resetCooldown>0)return;try{await sendResetCodeDirect(resetEmail);showToast('Code resent','success');setResetCooldown(60);}catch(err){setError(err instanceof Error?err.message:'Failed');}}} disabled={resetCooldown>0} className="text-[12px] text-rh-green hover:text-rh-green/80 disabled:text-white/80 transition-colors">{resetCooldown>0?`Resend in ${resetCooldown}s`:'Resend code'}</button><button type="button" onClick={()=>{setAuthMode('login');setError('');setResetEmail('');setResetCode('');setNewPassword('');setNewPasswordConfirm('');}} className="text-[12px] text-white hover:text-white transition-colors">Back to sign in</button></div>
                  </>) : (<>
                  <div><label htmlFor="auth-username" className="block text-[12px] font-medium text-white mb-1.5">Username</label><input id="auth-username" type="text" value={username} onChange={e=>setUsername(e.target.value)} onBlur={checkAndSwitchMode} className={ic} placeholder="e.g. nala_investor" autoComplete="username" autoCapitalize="none" autoCorrect="off" spellCheck="false" required /></div>
                  {authMode==='signup'&&<div><label htmlFor="auth-displayName" className="block text-[12px] font-medium text-white mb-1.5">Display Name</label><input id="auth-displayName" type="text" value={displayName} onChange={e=>setDisplayName(e.target.value)} className={ic} placeholder="How others will see you" autoComplete="name" required /></div>}
                  {authMode==='signup'&&<div><label htmlFor="auth-email" className="block text-[12px] font-medium text-white mb-1.5">Email</label><input id="auth-email" type={EMAIL_INPUT_TYPE} inputMode="email" value={landingEmail} onChange={e=>setLandingEmail(e.target.value)} className={ic} placeholder="you@example.com" autoComplete="email" autoCapitalize="none" required /></div>}
                  {authMode==='signup'&&<div><label htmlFor="auth-dob" className="block text-[12px] font-medium text-white mb-1.5">Date of Birth</label><input id="auth-dob" type="date" value={dateOfBirth} onChange={e=>setDateOfBirth(e.target.value)} className={ic} autoComplete="bday" required /></div>}
                  <div><div className="flex items-center justify-between mb-1.5"><label htmlFor="auth-password" className="block text-[12px] font-medium text-white">Password</label>{authMode==='login'&&<div className="flex items-center gap-3"><button type="button" tabIndex={-1} className="text-[11px] text-white/80 hover:text-white transition-colors" onClick={()=>{setAuthMode('forgot-username');setError('');}}>Forgot username?</button><button type="button" tabIndex={-1} className="text-[11px] text-white/80 hover:text-white transition-colors" onClick={()=>{setAuthMode('forgot-password');setError('');}}>Forgot password?</button></div>}</div><div className="relative"><input id="auth-password" type={showPassword?'text':'password'} value={password} onChange={e=>setPasswordValue(e.target.value)} className={`${ic} pr-11`} placeholder={authMode==='login'?'••••••••':'Min. 8 chars, upper/lower/number'} autoComplete={authMode==='login'?'current-password':'new-password'} required /><button type="button" onClick={()=>setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-white/80 hover:text-white transition-colors" tabIndex={-1}>{showPassword?<EyeOffIcon />:<EyeIcon />}</button></div></div>
                  {authMode==='signup'&&<div><label htmlFor="auth-confirm" className="block text-[12px] font-medium text-white mb-1.5">Confirm Password</label><div className="relative"><input id="auth-confirm" type={showConfirmPassword?'text':'password'} value={confirmPassword} onChange={e=>setConfirmPassword(e.target.value)} className={`${ic} pr-11`} placeholder="Re-enter password" autoComplete="new-password" required /><button type="button" onClick={()=>setShowConfirmPassword(!showConfirmPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-white/80 hover:text-white transition-colors" tabIndex={-1}>{showConfirmPassword?<EyeOffIcon />:<EyeIcon />}</button></div></div>}
                  ...
                  {authMode==='signup'&&<label className="flex items-start gap-2.5 cursor-pointer"><input type="checkbox" checked={acceptedTerms} onChange={e=>setAcceptedTerms(e.target.checked)} className="w-4 h-4 mt-0.5 rounded border-white/10 bg-white/5 text-rh-green accent-rh-green" /><span className="text-[12px] text-white/80 leading-tight">I agree to the{' '}<button type="button" onClick={()=>{setPrivacyTab('privacy');setShowPrivacyPolicy(true);}} className="text-white/80 hover:underline">Privacy Policy</button>{' & '}<button type="button" onClick={()=>{setPrivacyTab('terms');setShowPrivacyPolicy(true);}} className="text-white/80 hover:underline">Terms</button></span></label>}
                ...
                {(authMode==='login'||authMode==='signup') && OAUTH_ENABLED && (
                  <div className="mt-5 pt-4 border-t border-white/[0.04]">
                    <p className="text-[11px] text-white/80 text-center mb-3">Or continue with</p>
                    <div className="flex gap-2">
                      {GOOGLE_ENABLED && (
                        <button
                          ...
                          className="flex-1 flex items-center justify-center gap-2 py-2.5 px-3 bg-white/[0.04] border border-white/[0.08] rounded-lg hover:bg-white/[0.08] disabled:opacity-40 transition-all text-[12px] font-medium text-white min-h-[44px]"
                        >
                ...
                {(authMode==='login'||authMode==='signup')&&<div className={`${OAUTH_ENABLED ? 'mt-3' : 'mt-5'} pt-4 border-t border-white/[0.04] text-center text-[12px] text-white/80`}>{authMode==='login'?<>New to Nala? <button onClick={()=>setAuthMode(DIRECT_SIGNUP_ENABLED ? 'signup' : 'waitlist')} className="text-white hover:text-white">{DIRECT_SIGNUP_ENABLED ? 'Create an account' : 'Join the waitlist'}</button></>:<>Have an account? <button onClick={()=>setAuthMode('login')} className="text-white hover:text-white">Sign in</button></>}</div>}
              </>
            )}
          </div>
        </div>,
