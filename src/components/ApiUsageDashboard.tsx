...
export function ApiUsageDashboard({ onBack }: Props) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="flex gap-1 bg-gray-100 dark:bg-white/[0.04] rounded-lg p-0.5">
          {([7, 30, 90] as Period[]).map(p => (
            <button key={p} onClick={() => setPeriod(p)}
              className={`px-3 py-1 text-xs font-medium rounded-md transition-colors ${period === p ? 'bg-white dark:bg-white/10 text-rh-light-text dark:text-white shadow-sm' : 'text-rh-light-text dark:text-white hover:text-rh-light-text dark:hover:text-white'}`}>
              {p}d
            </button>
          ))}
        </div>
      ...
      {loading && !data && <div className="text-center py-12 text-rh-light-text dark:text-white">Loading...</div>}
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className={`${CARD} p-4`}>
              <p className="text-[10px] uppercase tracking-wider text-rh-light-text/70 dark:text-white/80 mb-1">Total Calls</p>
              <p className="text-2xl font-bold text-rh-light-text dark:text-white tabular-nums">{formatNumber(data.totals.calls)}</p>
            </div>
            <div className={`${CARD} p-4`}>
              <p className="text-[10px] uppercase tracking-wider text-rh-light-text/70 dark:text-white/80 mb-1">Total Cost</p>
              <p className="text-2xl font-bold text-rh-green tabular-nums">{formatCost(data.totals.costUsd)}</p>
            </div>
            <div className={`${CARD} p-4`}>
              <p className="text-[10px] uppercase tracking-wider text-rh-light-text/70 dark:text-white/80 mb-1">Input Tokens</p>
              <p className="text-2xl font-bold text-rh-light-text dark:text-white tabular-nums">{formatNumber(data.totals.inputTokens)}</p>
            </div>
            <div className={`${CARD} p-4`}>
              <p className="text-[10px] uppercase tracking-wider text-rh-light-text/70 dark:text-white/80 mb-1">Output Tokens</p>
            ...
            <div className="space-y-1.5">
              {days.map(([date, v]) => (
                <div key={date} className="flex items-center gap-3">
                  <span className="text-[11px] text-rh-light-text dark:text-white w-20 flex-shrink-0 tabular-nums">
                  ...
                  <span className="text-[11px] font-medium text-rh-light-text dark:text-white w-10 text-right tabular-nums">{v.calls}</span>
                  <span className="text-[10px] text-rh-light-text dark:text-white w-14 text-right tabular-nums">{formatCost(v.cost)}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className={`${CARD} p-5`}>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead>
                  <tr className="text-[10px] uppercase tracking-wider text-rh-light-text/70 dark:text-white/80">
                ...
                <tbody>
                  {features.map(([name, v]) => (
                    <tr key={name} className="border-t border-gray-100 dark:border-white/[0.04]">
                      ...
                      <td className="py-2 text-sm text-right text-rh-light-text dark:text-white/80 tabular-nums">{v.calls}</td>
                      <td className="py-2 text-sm text-right text-rh-light-text dark:text-white/80 tabular-nums">{formatNumber(v.inputTokens)}</td>
                      <td className="py-2 text-sm text-right text-rh-light-text dark:text-white/80 tabular-nums">{formatNumber(v.outputTokens)}</td>
