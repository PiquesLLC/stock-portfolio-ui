...
export function StockLogo({ ticker, size = 'sm', className = '' }: StockLogoProps) {
  ...
  const glassClasses = 'rounded-lg bg-gray-100 dark:bg-white/[0.06] backdrop-blur-md border border-gray-200/60 dark:border-white/[0.1] shadow-[inset_0_1px_0_0_rgba(255,255,255,0.06)]';
  ...
  return (
    <div className={`${s.container} ${glassClasses} flex items-center justify-center shrink-0 overflow-hidden ${className}`}>
      <img
        src={url}
        alt={`${ticker} logo`}
        className={`${s.img} object-contain invert hue-rotate-180 dark:invert-0 dark:hue-rotate-0`}
